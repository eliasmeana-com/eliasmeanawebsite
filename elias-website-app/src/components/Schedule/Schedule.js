...
const scheduleData = require('../../classData/scheduleData.json').scheduleData;
...
const getMonthViewRange = (date) => {
    ...
    // Start from Sunday before (or on) the first day of month
    const start = new Date(firstDay);
    start.setDate(firstDay.getDate() - firstDay.getDay());

    // End at Saturday after (or on) the last day of month
...
const fetchSchedule = async (targetDate, view) => {
    ...
    const activeClasses = scheduleData.filter(classItem => {
        const classStart = new Date(classItem.startDate);
        const classEnd = new Date(classItem.endDate);
    ...
    if (view === 'week') {
        ...
        activeClasses.forEach(classItem => {
            classItem.times.forEach(timeSlot => {
                const day = timeSlot.day;
                if (!scheduleByDay[day]) scheduleByDay[day] = { day, classes: [] };

                const [startTime, endTime] = timeSlot.timeRange.split('-');
                scheduleByDay[day].classes.push({
                    ...
                    instructor: timeSlot.instructor || 'TBD',
                    description: timeSlot.description || '',
                    courseHomePage: timeSlot.courseHomePage || '',
                });
            });
        });
    ...
    activeClasses.forEach(classItem => {
        classItem.times.forEach(timeSlot => {
            const dayIndex = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].indexOf(timeSlot.day);

            // Use a separate variable to iterate dates here
            for (let dayIter = new Date(start); dayIter <= end; dayIter.setDate(dayIter.getDate() + 1)) {
                if (dayIter.getDay() === dayIndex) {
                    const dateKey = dayIter.toISOString().split('T')[0];
                    const [startTime, endTime] = timeSlot.timeRange.split('-');
                                ...
                                <li key={idx} className="class-item">
                                    ...
                                    {cls.courseHomePage && (
                                        <p className="class-description">
                                            <a href={cls.courseHomePage} target="_blank" rel="noopener noreferrer">Website</a>
                                        </p>
                                    )}
                                </li>
                            ))}
